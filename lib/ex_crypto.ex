defmodule ExCrypto do
  ...
  `encrypt(key, clear_text, %{initialization_vector: initialization_vector})`:

  Will use the given IV, and return a tuple containing the `initialization_vector` and the `cipher_text`.

  `encrypt(key, auth_data, clear_text)`:

  Will generate a random 128-bit IV, and return a tuple containing the `initialization_vector`, the `cipher_text` and the `cipher_tag`.
